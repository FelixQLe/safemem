#[cfg(test)]
mod tests {
    use safemem::Buffer; // Import Buffer from the safemem crate

    #[test]
    fn test_buffer_overflow_1() {
        println!("\n=== Buffer Overflow Test 1 (Small Overflow) ===");
        let mut buffer = Buffer::new(4).expect("Failed to create buffer");
        ...
        match buffer.append(b"Hello") {
            Ok(_) => println!("Unexpected success appending 'Hello'"),
            Err(e) => println!("{}", e),
        }
    ...
    fn test_buffer_overflow_2() {
        println!("\n=== Buffer Overflow Test 2 (Large Overflow) ===");
        let mut buffer = Buffer::new(5).expect("Failed to create buffer");
    ...
    fn test_double_free_1() {
        println!("\n=== Double Free Test 1 (Basic Double-Free) ===");
        let mut buffer = Buffer::new(10).expect("Failed to create buffer");
    ...
    fn test_double_free_2() {
        println!("\n=== Double Free Test 2 (Free-and-Reuse) ===");
        let mut buffer = Buffer::new(10).expect("Failed to create buffer");
        println!("Created buffer");
        ...
        let mut new_buffer = Buffer::new(10).expect("Failed to create buffer");
    ...
    fn test_use_after_free_1() {
        println!("\n=== Use After Free Test 1 (Simple Reuse) ===");
        let mut buffer = Buffer::new(10).expect("Failed to create buffer");
        println!("Created buffer");
    ...
    fn test_use_after_free_2() {
        println!("\n=== Use After Free Test 2 (Double Append) ===");
        let mut buffer = Buffer::new(10).expect("Failed to create buffer");
        println!("Created buffer");
